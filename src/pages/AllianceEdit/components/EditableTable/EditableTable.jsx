...
import AllianceService from '../../../../service/AllianceService';
...
export default class EditableTable extends Component {
  ...
  generateData() {
    AllianceService.getList()
      .then((response) => {
        if (response.data.success) {
          console.log('---展教联盟列表查询成功---');
          // return response.data.allianceList;
          this.setState({ dataSource: response.data.allianceList });
        } else {
          console.log('---展教联盟列表查询失败---');
          this.setState(
            {
              dataSource: [
                {
  ...
  deleteItem = (index) => {
    AllianceService.deleteAlliance(this.state.dataSource[index])
      .then((response) => {
        if (response.data.success) {
  ...
  changeDataSource = (index, valueKey, value) => {
    ...
    AllianceService.updateAlliance(this.state.dataSource[index])
      .then((response) => {
        if (response.data.success) {
          // this.state.dataSource[index][valueKey] = value;
          this.setState({
